...
public class TestGameManager {
    ...
    public void testEnergiaNegativa() {
        ...
        Assert.assertEquals("[[1, João, P, 46, 5..6], [3, Manuel, T, 155, 1..3]]",
                Arrays.deepToString(game.getPlayersInfo()));

        game.moveCurrentPlayer(6, true);
        game.moveCurrentPlayer(0, true);
        Assert.assertEquals("[[1, João, P, 22, 5..6], [3, Manuel, T, 160, 1..3]]",
                Arrays.deepToString(game.getPlayersInfo()));

        game.moveCurrentPlayer(6, true); //não tem energia
        game.moveCurrentPlayer(0, true);
        Assert.assertEquals("[[1, João, P, 22, 5..6], [3, Manuel, T, 165, 1..3]]",
                Arrays.deepToString(game.getPlayersInfo()));

        game.moveCurrentPlayer(4, true);
        game.moveCurrentPlayer(0, true);
        Assert.assertEquals("[[1, João, P, 6, 5..6], [3, Manuel, T, 170, 1..3]]",
                Arrays.deepToString(game.getPlayersInfo()));

        Assert.assertEquals("CAUGHT_FOOD",
                String.valueOf(game.moveCurrentPlayer(1, true).code()));

        game.moveCurrentPlayer(0, true);
        Assert.assertEquals("[[1, João, P, 42, 5..6], [3, Manuel, T, 175, 1..3]]",
                Arrays.deepToString(game.getPlayersInfo()));
    }
